...
import {
	statefulFAQExtractionPrompt,
	removeSimilarFAQsPrompt,
...
export async function extractFAQs(
	...
	if (useContinue === true) {
		...
		const chain = new ConversationChain({
			...
			prompt: statefulFAQExtractionPrompt(repTranscript),
			llm: new ChatOpenAI({
	...
	await fs.writeFile(`./scripts/${fileNameWithoutExtension}.txt`, fullResponse);
