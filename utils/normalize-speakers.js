...
export async function normalizeSpeakers(arr) {
	...
	let updatedData = updateSpeakers(arr, nonPrimaryIndexes, determinedSpeakers);
...
function updateSpeakers(originalArray, nonPrimarySpeakerIndexes, newSpeakers) {
	return originalArray.map((obj, index) => {
		...
		if (nonPrimarySpeakerIndexes.includes(index)) {
			...
			if (newSpeaker !== undefined && (newSpeaker === 1 || newSpeaker === 2)) {
				return {
					...obj,
					speaker: newSpeaker,
				};
...
function combineAdjacentSameSpeaker(arr) {
	let combinedArr = [];
	let combinedObj = {};

	for (let i = 0; i < arr.length; i++) {
		if (i === 0 || arr[i].speaker !== arr[i - 1].speaker) {
			if (i !== 0) {
				combinedArr.push(combinedObj);
			}
			combinedObj = {
				speaker: arr[i].speaker,
				transcript: arr[i].transcript,
			};
		} else {
			combinedObj.transcript += " " + arr[i].transcript;
		}
	}

	combinedArr.push(combinedObj); // push last combined transcript object
	return combinedArr;
}
